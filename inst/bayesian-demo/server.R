...
shinyServer(function(input, output, session) {
    ...
    withProgress(message = 'Running Bayesian Model', detail = "reading data", value = 0, {
      ...
      for (grade_i in seq_along(names(data_list))) {
        ...
        grade_data <- data_list[[grade]]
          ...
          if (file.exists('demo_model.rds')) {
            lm1 <- readRDS('demo_model.rds')
          }
          else {
            lm1 <- try(stanlm(formula = as.formula(myformula), data = grade_data))
            saveRDS(lm1, 'demo_model.rds')
          }
        #}else{
  ...
  observeEvent(input$plot_click,{
    updateNumericInput(session, "Q_BD_1",
  ...
  ci <- reactive({
    ...
    if (!is.null(model)) {
      model <- updateci(model, input$Q_BD_3 / 100)
      whichParameter <- which(model$tbl@coef.names == input$trt_var)
  ...
  output$output_by_grade <- renderUI({
    ...
    output$posterior_plot <- renderPlot({
      if (input$go == 0)
        ...
        p <- posteriorplot(model = lmupdated()[[1]],
                           parameter = input$trt_var , 
                           cutoff = input$Q_BD_1,
                           credibleIntervalWidth = input$Q_BD_3 / 100,
                           lessthan = db$lessthan)
      return(p)
