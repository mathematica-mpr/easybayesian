...
posteriorplot <- function(model, parameter, cutoff=0,
  ...
  p <- ggplot(
       ...
       geom_area(
         ...
         alpha = 0.5,
         fill = "darkgreen") +
      # Remove credible interval at bottom of plot - leaving here in case it becomes useful in the future
      # custom_range(
      #   c(credibleInterval[1, whichParameter],
      #     credibleInterval[2, whichParameter]),
      #     pointEstimate[whichParameter]) +
       annotate("text",
                x = mid,
                y = (density(df.plot$density, from=mid, to=mid, n=1)$y) / 2,
                label=prob,
                fontface="bold",
                family="sans") +
    theme_mpr() +
